...
fun includes (xs: int list, y: int) = 
    if null xs then false
    else
        if hd xs = y 
        then true
        else includes(tl xs, y)

(* Not the most efficient implementation, the time complexity will be quadratic, O(n^2), because for each element in orig_list, we are potentially scanning all elements in uniq_list.  *)
fun unique (uniq_list: int list, orig_list: int list) = 
    if null orig_list then uniq_list
    else
        if includes(uniq_list, hd orig_list)
        then unique(uniq_list, tl orig_list)
        else unique(uniq_list@[hd orig_list], tl orig_list)
